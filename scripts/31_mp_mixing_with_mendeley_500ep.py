...
print("==================================================")
print("  500-EPOCH DATASET MIXING: MENDELEY + MP (10% & 20%) -> HELD-OUT MP TEST")
print("==================================================")
...
def save_500ep_loss_curve(train_losses, test_losses, title, filename):
    plt.figure(figsize=(9, 5))
    epochs = range(1, len(train_losses) + 1)
    plt.plot(epochs, train_losses, label='Training MAE Loss', color='blue', linewidth=1.5)
    if test_losses:
        plt.plot(epochs, test_losses, label='Held-Out Materials Project Test MAE', color='red', linewidth=1.5)
...
np.random.seed(42)
mp_perm = np.random.permutation(len(mp_dataset))

# -----------------------------------------------------------------------------
# EXPERIMENT A: Mendeley (24,327) + 10% MP (1,453) -> Test on remaining 90% MP (13,077)
# -----------------------------------------------------------------------------
idx_10 = int(0.10 * len(mp_dataset))
mp_10_mix = [mp_dataset[i] for i in mp_perm[:idx_10]]
mp_90_test = [mp_dataset[i] for i in mp_perm[idx_10:]]

train_set_10 = mendeley_dataset + mp_10_mix
train_loader_10 = DataLoader(train_set_10, batch_size=16, shuffle=True)
test_loader_90 = DataLoader(mp_90_test, batch_size=16, shuffle=False)

print(f"\n--- EXPERIMENT A: Mendeley ({len(mendeley_dataset)}) + 10% MP ({len(mp_10_mix)}) -> Test 90% MP ({len(mp_90_test)}) ---")
...
for epoch in range(1, 501):
    ...
    if epoch % 50 == 0 or epoch == 1 or epoch == 500:
        ...
        print(f"Mix 10% MP | Epoch {epoch:03d}/500 | Train MAE: {tr_mae:.4f} e | Held-Out 90% MP MAE: {te_mae:.4f} e")
    else:
        test_losses_90.append(test_losses_90[-1] if test_losses_90 else tr_mae)

final_mix10_mae = test_losses_90[-1]
save_500ep_loss_curve(train_losses_10, test_losses_90, "Mendeley + 10% MP Mixing -> Test on 90% Held-Out MP", "loss_curve_mix10_mp_tested_on_90mp_500ep.png")

# -----------------------------------------------------------------------------
# EXPERIMENT B: Mendeley (24,327) + 20% MP (2,906) -> Test on remaining 80% MP (11,624)
# -----------------------------------------------------------------------------
idx_20 = int(0.20 * len(mp_dataset))
mp_20_mix = [mp_dataset[i] for i in mp_perm[:idx_20]]
mp_80_test = [mp_dataset[i] for i in mp_perm[idx_20:]]

train_set_20 = mendeley_dataset + mp_20_mix
train_loader_20 = DataLoader(train_set_20, batch_size=16, shuffle=True)
test_loader_80 = DataLoader(mp_80_test, batch_size=16, shuffle=False)

print(f"\n--- EXPERIMENT B: Mendeley ({len(mendeley_dataset)}) + 20% MP ({len(mp_20_mix)}) -> Test 80% MP ({len(mp_80_test)}) ---")
...
for epoch in range(1, 501):
    ...
    if epoch % 50 == 0 or epoch == 1 or epoch == 500:
        ...
        print(f"Mix 20% MP | Epoch {epoch:03d}/500 | Train MAE: {tr_mae:.4f} e | Held-Out 80% MP MAE: {te_mae:.4f} e")
    else:
        test_losses_80.append(test_losses_80[-1] if test_losses_80 else tr_mae)

final_mix20_mae = test_losses_80[-1]
save_500ep_loss_curve(train_losses_20, test_losses_80, "Mendeley + 20% MP Mixing -> Test on 80% Held-Out MP", "loss_curve_mix20_mp_tested_on_80mp_500ep.png")

# Save Summary CSV
mixing_results_csv = os.path.join(output_dir, "mp_dataset_mixing_results.csv")
pd.DataFrame([
    {
        'Experiment': 'Mendeley + 10% MP Mixing',
        'Train_Graphs': len(train_set_10),
        'Test_Graphs': len(mp_90_test),
        'Epochs': 500,
        'Final_Train_MAE_e': train_losses_10[-1],
        'Final_Test_MP_MAE_e': final_mix10_mae,
        'Loss_Plot': 'loss_curve_mix10_mp_tested_on_90mp_500ep.png'
    },
    {
        'Experiment': 'Mendeley + 20% MP Mixing',
        'Train_Graphs': len(train_set_20),
        'Test_Graphs': len(mp_80_test),
        'Epochs': 500,
        'Final_Train_MAE_e': train_losses_20[-1],
        'Final_Test_MP_MAE_e': final_mix20_mae,
        'Loss_Plot': 'loss_curve_mix20_mp_tested_on_80mp_500ep.png'
    }
]).to_csv(mixing_results_csv, index=False)

print(f"\nSaved MP dataset mixing results summary to: {mixing_results_csv}")
print("==================================================")
