...
print("==================================================")
print("  TRAINING BM1 MODEL FROM SCRATCH (RANDOM NOISE)")
print("==================================================")

data_dir = "data"
output_dir = "outputs"  # Output directory for saved models and plots
xyz_file = "data/BEC_ZrO2.xyz"
bm1_model_path = "data/BM1.pt"

# Ensure outputs directory exists
os.makedirs(output_dir, exist_ok=True)

# Step 1: Extract Ground-Truth Targets from BEC_ZrO2.xyz
...
# Step 3: Load BM1 Architecture & Re-initialize Weights to Random Noise
print("\n[Step 3/5] Re-initializing BM1 Weights to Random Noise (Training From Scratch)...")
model = torch.jit.load(bm1_model_path, map_location='cpu')
...
print("\n[Step 4/5] Starting BM1 Training From Scratch (15 Epochs)...")
print("-" * 50)
t0 = time.time()
...
t1 = time.time()
print("-" * 50)
print(f"-> BM1 Scratch Training Complete in {t1 - t0:.2f} seconds!")

# Step 5: Save Custom BM1 Weights & Plot Loss Curve to outputs/ folder
print("\n[Step 5/5] Saving Trained BM1 Model Weights & Generating Loss Plot to outputs/...")

save_path = os.path.join(output_dir, "BM1_trained_scratch.pt")
...
plt.plot(range(1, epochs + 1), loss_history, marker='o', color='#e05a47', linewidth=2, label='BM1 Scratch Training Loss')
plt.title('BM1 Training From Scratch Loss Curve (Random Weight Start)', fontsize=12, fontweight='bold')
plt.xlabel('Epoch', fontsize=11)
plt.ylabel('L1 Loss Error (e)', fontsize=11)
plt.legend()
plt.tight_layout()

plot_path = os.path.join(output_dir, "bm1_scratch_training_loss.png")
plt.savefig(plot_path)
print(f"  - Saved scratch training loss plot image to: '{plot_path}'")

print("\n==================================================")
print("  ALL BM1 SCRATCH TRAINING STEPS FINISHED SUCCESSFULLY!")
print("==================================================")
